...
const GITHUB_URL = 'https://github.com/IceyWalterWhite/rm-multiview';

// 底栏常驻：GitHub 入口任何环境都挂；备案号只属于备案域名——
// Vercel 预览域（*.vercel.app）、IP 直连、本机开发都不悬挂
export function IcpFooter({ hostname = location.hostname }: { hostname?: string }) {
  const filed = /(^|\.)rmlive\.cn$/.test(hostname);
  return (
